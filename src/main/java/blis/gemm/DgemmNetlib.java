...
final class DgemmNetlib {
    ...
    static void dgemm(boolean notA, boolean notB, int m, int n, int k, double alpha, double[] a, int _a_offset, int lda,
        ...
        dgemm(notB, m, n, k, a, lda, b, ldb, c, ldc);
    }
     ...
     *            rows of the matrix op(B)
     */
    static void dgemm(boolean notBTransposed, int m, int n, int k, double[] a, int lda, double[] b, int ldb, double[] c, int ldc) {
            ...
            for (int o = n; o > 0; o--) {
                ...
                for (int p = k; p > 0; p--) {
                    ...
                    if (tmp != 0.0) {
                        ...
                        // 1. Main loop, manually unrolled into 12 steps
                        while (q >= UNROLL) {
