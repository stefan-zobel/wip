...
public final class CancellationToken {
    ...
    private static final AtomicReferenceFieldUpdater<CancellationToken, OnCancelNode> ON_CANCEL_NODES_UPDATER = AtomicReferenceFieldUpdater
            .newUpdater(CancellationToken.class, OnCancelNode.class, "onCancelNodes");
    ...
    /**
     * Registers a {@link Runnable} that will be called when this
     * CancellationToken is canceled.
     *
     * This operation of registering the {@link Runnable} is non-blocking and
     * thread-safe. If the token is already in the cancelled state then the
     * provided Runnable will be immediately executed.
     *
     * This is O(1) in best case and O(k) in worst, where k is number of
     * concurrent threads in case of race.
     *
     * @param onCancel
     *            The {@link Runnable} to be executed when the CancellationToken
     *            is canceled.
     */
    public void registerOnCancel(Runnable onCancel) {
        ...
        boolean added = tryAddOnCancelNode(node);
        if (!added) {
            // Couldn't add since the list was frozen as a result of
            // cancellation, invoke this Runnable Callback immediately.
            node.invokeOnCancel();
        }
    }
     ...
     *
     * This is O(1) in best case and O(k) in worst, where k is number of
     * concurrent threads in case of race in registering Callback.
     *
     * @param id
     *            The registration id for the {@link Runnable} to register.
     * @param onCancel
     *            The {@link Runnable} to be executed when the CancellationToken
     *            is canceled.
     */
     ...
     *
     * @param id
     *            The id of the {@link Runnable} to unregister.
     */
    public void unregisterOnCancel(String id) {
        if (this == CancellationToken.NONE) {
            return;
        }
        OnCancelNode itr = onCancelNodes;
        // Step_1: Locate the node and mark it as logically deleted.
        //
        while (itr != null) {
            if (onCancelNodes == OnCancelNode.FROZEN) {
        ...
        do {
            ...
            OnCancelNode current = onCancelNodes; // re-fetch the volatile head
                                                  // for each retry.
            ...
            while (current != null) {
                ...
                if (current.isDeleted()) {
                    ...
                    if (predecessor == null) {
                        // The 'current' node has no 'predecessor' hence it's
                        // head, try CAS head with 'successor'.
                        if (!ON_CANCEL_NODES_UPDATER.compareAndSet(this, current, successor)) {
                            // Raced with
                            // 1. another thread calling
                            // registerOnCancelCallback.
                        ...
                        if (predecessor.isDeleted()) {
                            // Raced with another thread that unlinked
                            // 'predecessor', need to retry.
                            hadRace = true;
                            break;
                        }
                    }
                } else {
                    // We aren't un-linking 'current' node, update
                    // 'predecessor'.
                    predecessor = current;
                }
                current = successor;
     ...
     *
     * @param newNode
     *            The node holding onCancel Callback.
     *
     * @return false if the list got frozen hence onCancel Callback node is not
     *         added to the list, true otherwise.
     */
    private boolean tryAddOnCancelNode(OnCancelNode newNode) {
        // fetch the head pointing to the callback list.
        OnCancelNode headNode = onCancelNodes;
        ...
        do {
            ...
            // insert the new node to the list by making it the head and it's
            // next referencing to the old list.
            // Do this only if the head was not updated since last fetch, it is
            // possible that another thread added a new node in mean time, in
            // that case, this thread's local reference, headNode, to the head
            // is outdated.
            if (!ON_CANCEL_NODES_UPDATER.compareAndSet(this, headNode, newNode)) {
                // head was changed since last fetched, re-fetch.
                headNode = onCancelNodes;
            } else {
    ...
    /**
     * Atomically mark the onCancel Callback list as frozen and get the
     * Callbacks in the order those were added through
    ...
    private OnCancelNode freezeAndGetCallbackNodes() {
        OnCancelNode current = freezeCallbackNodes();
    ...
    private void invokeCallbacks() {
        OnCancelNode next = freezeAndGetCallbackNodes();
        while (next != null) {
            next.invokeOnCancel();
            next = next.next;
        }
    ...
    private static final class OnCancelNode {
        ...
        /**
         * Mark the node as deleted hence not a part of the OnCancel Callback
         * list any more.
         */
        void markDeleted() {
            isDeleted = true;
