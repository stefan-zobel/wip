 ...
 *
 * Serial warnings are suppressed throughout because all implementation classes
 * use a serial proxy and thus have no need to declare serialVersionUID.
 */
@SuppressWarnings("serial")
final class ImmutableCollections {
    /**
     * A "salt" value used for randomizing iteration order. This is initialized
     * once and stays constant for the lifetime of the JVM. It need not be truly
     * random, but it needs to vary sufficiently from one run to the next so
     * that iteration order will vary between JVM runs.
     */
    ...
    /**
     * The reciprocal of load factor. Given a number of elements to store,
     * multiply by this factor to get the table size.
     */
    ...
    static final class ListN<E> extends AbstractImmutableList<E> implements Serializable {
        ...
        static final List<?> EMPTY_LIST = new ListN<Object>(new Object[0]);
     ...
     * larger than the size (the number of contained elements) so that at least
     * one null is always present.
     * 
     * @param <E>
     *            the element type
     */
     ...
     * also be strictly larger than the size (the number of key-value pairs
     * contained in the map) so that at least one null key is always present.
     * 
     * @param <K>
     *            the key type
     * @param <V>
...
final class ColSer implements Serializable {
    ...
    /**
     * Indicates the type of collection that is serialized. The low order 8 bits
     * have the value 1 for an unmodifiable {@code List}, 2 for an unmodifiable
     * {@code Set}, and 3 for an unmodifiable {@code Map}. Any other value
     * causes an {@link InvalidObjectException} to be thrown. The high order 24
     * bits are zero when an instance is serialized, and they are ignored when
     * an instance is deserialized. They can thus be used by future
     * implementations without causing compatibility issues.
     *
     * <p>
     * The tag value also determines the interpretation of the transient
     * {@code Object[] array} field. For {@code List} and {@code Set}, the
     * array's length is the size of the collection, and the array contains the
     * elements of the collection. Null elements are not allowed. For
     * {@code Set}, duplicate elements are not allowed.
     *
     * <p>
     * For {@code Map}, the array's length is twice the number of mappings
     * present in the map. The array length is necessarily even. The array
     * contains a succession of key and value pairs:
     * {@code k1, v1, k2, v2, ..., kN, vN.} Nulls are not allowed, and duplicate
    ...
    /**
     * Reads objects from the stream and stores them in the transient
     * {@code Object[] array} field.
     *
     * @serialData A nonnegative int, indicating the count of objects, followed
     *             by that many objects.
     *
     * @param ois
    ...
    /**
     * Writes objects to the stream from the transient {@code Object[] array}
     * field.
     *
     * @serialData A nonnegative int, indicating the count of objects, followed
     *             by that many objects.
    ...
    /**
     * Creates and returns an unmodifiable collection from this proxy class. The
     * instance returned is created as if by calling one of the static factory
     * methods for <a href="Lists.html#unmodifiable">List</a>,
     * <a href="Maps.html#unmodifiable">Map</a>, or
     * <a href="Sets.html#unmodifiable">Set</a>. This proxy class is the serial
     * form for all unmodifiable collection instances, regardless of
     * implementation type. This is necessary to ensure that the existence of
     * any particular implementation type is kept out of the serialized form.
     *
     * @return a collection created from this proxy object
     * @throws InvalidObjectException
     *             if the tag value is illegal or if an exception is thrown
     *             during creation of the collection
