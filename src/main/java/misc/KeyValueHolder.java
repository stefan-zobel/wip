...
/**
 * An immutable container for a key and a value, suitable for use in creating
 * and populating {@code Map} instances.
 *
 * <p>
 * This is a
 ...
 * ways: it is not serializable, it is final, and its key and value must be
 * non-null.
 *
 * @param <K>
 *            the key type
...
final class KeyValueHolder<K, V> implements Map.Entry<K, V> {
     ...
     * keys and values are equal. Note that key and value are non-null, so
     * equals() can be called safely on them.
     */
    @Override
    ...
    /**
     * Returns the hash code value for this map entry. The hash code is
     * {@code key.hashCode() ^ value.hashCode()}. Note that key and value are
     * non-null, so hashCode() can be called safely on them.
     */
    @Override
    public int hashCode() {
     ...
     * equals character ("{@code =}") followed by the string representation of
     * this entry's value.
     *
